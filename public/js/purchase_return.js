...
function UpdateItemID(obj){
	    ...
	    lastrow = objek.substr(5,objek.length-3);
	     ...
	     var PCode = $('#pcode'+lastrow).val();
			  ...
			  $.ajax({
					url: url+"index.php/transaksi/purchase_return/satuan/"+PCode+"/",
					data: {pcode:PCode},
					type: "POST",
					dataType: 'html',					
...
function deleteTrans(nodok,url)
{
	...
	if (r==true)
	{
		window.location = url+"index.php/transaksi/purchase_return/delete_trans/"+nodok;	
	}
	else
	{
  		return false;
...
function deleteDetail(sid,pcode,nodok,url)
{
	...
	if (r==true)
	{
		window.location = url+"index.php/transaksi/purchase_return/delete_detail/"+sid+"/"+pcode+"/"+nodok+"";	
	}
	else
	{
  		return false;
...
function pickThis(obj)
{
	base_url = $("#base_url").val();
		...
		windowOpener(525, 1000, 'Cari Purchase', url, 'Cari Purchase')
...
function pickThis2(obj)
{
	base_url = $("#base_url").val();
	    ...
	    objek = obj.id;
		id = parseFloat(objek.substr(9,objek.length-9));
		url = base_url+"index.php/pop/pop_up_cari_purchase_order/index/0/1/";
		windowOpener(525, 1000, 'Cari Purchase No', url, 'Cari Purchase No')
...
{
    url = "index.php/transaksi/purchase_return/vewPrint/" + escape(nodok);
