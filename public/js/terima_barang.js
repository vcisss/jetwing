...
function CatatLokasi(){ $("#gudang").val($("#namagudang").val()) }
...
//alert("ok");
	...
	if (code == 13) { //checks for the escape key
		...
		if(flag=='pcode'){
			...
			sumberMO=	document.querySelector('input[name="sumberMO"]:checked').value;
			//sumber = $("input[@name='sumberMO']:checked").val();
			if(sumber=="O")
			{
...
function findSupplier()
{
	if(cekoption("gudang","Memasukkan Kode Gudang"))
	{
		if(cekoption("kdsupplier","Memasukkan Kode Supplier")){
			...
			$.post(base_url+"index.php/transaksi/terima_barang/getSupplier",{ kdsupplier:kdsupplier },
			function(data){
				if(data!="")
				{	
						result = data.split('*_*');
...
function ubahsumberMO()
{
	sumberMO=	document.querySelector('input[name="sumberMO"]:checked').value;
	//sumberMO = $("input[@name='sumberMO']:checked").val();
	$("#hiddensumberMO").val(sumberMO);
	if(sumberMO=="M")
	{
	...
	else if(sumberMO=="O")
	{
		...
		$("#noorder").attr("readonly", "");
		$("#noorder").attr("disabled", "");*/
	}
}
...
function getOrder()
{
	if(cekoption("noorder","Memasukkan No Order")){
		if(validateForm("noorder","hiddennoorder","No Order")){
			noorderan = $("#noorder").val();
			base_url = $("#baseurl").val();
			$.post(base_url+"index.php/transaksi/terima_barang/getsumber",{ order:noorderan },
			function(data){
			    //alert(data);
				if(data=="")
				{
					alert("Purchase Order Tidak Ditemukan\nPeriksa Kembali Nomor Purchase Order");
					$("#noorder").focus();
					$("#hiddennoorder").val("");
				}
				else{
...
{
	base_url = $("#baseurl").val();
	url = base_url+"index.php/pop/porder/index/";
...
//alert(param.length);
	for(x=0;x<(param.length)-1;x++)
	{
		...
		if(nilai[21]!="")
		  $("#satuan"+baris).append("<option value='1|"+nilai[21]+"'>"+nilai[22]+"</option>"); //wieok
		if(nilai[23]!="")
		  $("#satuan"+baris).append("<option value='2|"+nilai[23]+"'>"+nilai[24]+"</option>");
		if(nilai[25]!="")
		  $("#satuan"+baris).append("<option value='3|"+nilai[25]+"'>"+nilai[26]+"</option>");
	...
	jQuery("input[name='sumberMO']").each(function(i) {
			...
			if(cekoption("pcode"+id,"Memasukkan Kode Barang")){
				...
				if(pcode!=pcodesave)
				{
					...
					if(!dobel){
						...
						kdgroupext = $("#kdgroupext").val();
						$.post(base_url+"index.php/transaksi/terima_barang/getPCode",{pcode:pcode,kdgroupext:kdgroupext},
						function(data){
							if(data!="")
							{	
								//alert(data);
...
function InputDisc2(id,from)
{
	if(cekoption("pcode"+id,"Memasukkan Kode Barang"))
	{
		...
		//alert(totalb+" "+batas);
		//if(Math.round(totalb)<Math.round(batas))
		//   alert("Silahkan cek harga beli");
		//else
		//{
			if(from=="enter"){
			   saveThis(id);
...
function ceksumberorder()
{
	...
	if(sumberMO=="O" && hiddenorder=="")
	{
		alert("Anda Belum Meng-enter No Order");
		$("#noorder").focus();
		return false;
	}
	else if(sumberMO=="O"  && hiddenorder!="")
	{
		if(validateForm("noorder","hiddennoorder","No Order"))
			return true;
		return false;
...
function saveItem(id)
{
	if($("#transaksi").val()=="no"){
		...
		//alert(gudang);
		//alert(kdsupplier);
		$.post(base_url+"index.php/transaksi/terima_barang/save_new_item",{ 
			flag:flag,no:no,tgl:tgl,kdsupplier:kdsupplier,ket:keterangan,jumlah:jumlah,total:total,
			gudang:gudang,tgljto:tgljto,sumber:sumber,sumberMO:sumberMO,
			noorder:noorder,pcode:pcode,extcode:extcode,
			qty:qty,pcodesave:pcodesave,satuan:satuan,hargab:hargab,disc1:disc1,disc2:disc2,jumlahb:jumlahb,ppnb:ppnb,
			totalb:totalb,konversi:konversi,satuanst:satuanst,nilsatuan:nilsatuan,nilsatuanst:nilsatuanst,
			kdkategori:kdkategori,kdbrand:kdbrand,jumlah:jumlah,ppn:ppn,total:total,top:top,
			nilaippn:nilaippn,kdgroupext:kdgroupext
		},
		function(data){
...
function deleteItem(pcode)
{
	if($("#transaksi").val()=="no"){
		...
		$("#transaksi").val("yes");
		base_url = $("#baseurl").val();
		$.post(base_url+"index.php/transaksi/terima_barang/delete_item",{ 
			no:no,sumberMO:sumberMO,noorder:noorder,pcode:pcode,gudang:gudang,jumlah:jumlah,ppn:ppn,nilaippn:nilaippn,total:total},
		function(data){
			$("#transaksi").val("no");
		});
	}
