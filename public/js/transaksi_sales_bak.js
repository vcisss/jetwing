...
function getCodeForSales(e,row,url,action)
{
    ...
    function barisPOS(url){
        ...
        dt2 = eval(dt);
        ttl = dt2.length;
		var discountspecial = 0;
                notrans = dt2[0]['NoTrans'];
		pcode = dt2[0]['PCode'];
		...
		for(index=1;index<=lastRow;index++)
		{
			...
			*/
                        discs = parseFloat($("#disc" + index).val());
                        discasli    = parseFloat($("#tempdisc" + index).val());
                       if(discs!=discasli) {
                           totalspecialdisc += parseFloat($("#qty" + index).val()) * parseFloat($("#harga" + index).val());
                       }
			...
			//alert(index);
                ...
                if(totalspecialdisc>=100000) {
                    for(index=1;index<=lastRow;index++) {
                        tempdiscabc = $("#tempdisc" + index).val();  
                        if(parseFloat($("#disc" + index).val())==0) {
                            $("#disc" + index).val(tempdiscabc);
                            $("#ldisc" + index).text(tempdiscabc);
                            $("#tempdisc" + index).val(0);
                     }
                    }
                    netto = 0;
                    totaldisc = 0;
                    ...
                    var listnilaidisc="";
                    for(index=1;index<=lastRow;index++) {
                        nettofinal  = ( parseFloat($("#harga" + index).val()) * parseFloat($("#qty" + index).val()) ) - parseFloat($("#disc" + index).val()) ;

                       document.getElementById('ljumlahnetto'+index).innerHTML = nettofinal;
                       netto = netto + parseFloat($("#ljumlahnetto" + index).text());
                       $("#netto" + index).val(nettofinal);
			totaldisc = totaldisc + parseFloat($("#ldisc" + index).text()); 
                        listpcode = listpcode + $("#pcode" + index).val() + "##";
			listqty = listqty + $("#qty" + index).val() + "##";
			listharga = listharga + $("#harga" + index).val() + "##";
			listdisc = listdisc + $("#disc" + index).val() + "##";
		...
		//alert(tgltrans);
		//alert(kassa);
		if((total+totaldisc>=500000)&&(tgltrans=='2016-11-09')&&(kassa<='26'))
		{
			//wieok
    ...
    function cloneRow(nbaru) {
		...
		*/
		...
		$("#nilaidisc" + nomor, clonedRow).attr( { "id" : "nilaidisc" + nbaru, "value" : 0} );
                ...
                $("#tempdisc" + nomor, clonedRow).attr( { "id" : "tempdisc" + nbaru, "value" : 0} );
