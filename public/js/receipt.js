function loading()
{
	...
	//$('#tglcair').datepicker({ dateFormat: 'dd-mm-yy',mandatory: true,showOn: "both", buttonImage: base_url+ "public/images/calendar.png", buttonImageOnly: true } );
...
function findkdrekening(id)
{
	...
	if(cekheader())
	{
		if(cekoption("kdrekening"+id,"Memasukkan Kode Rekening")){
			...
			$.post(base_url+"index.php/keuangan/receipt/getkdrekening",{ kdrekening:kdrekening},
			function(data){
				if(data!=""){
				    result = data.split('*-*');
					kode = result[0];
...
function saveItem(id)
{
	if($("#transaksi").val()=="no"){
		...
		jumlahreceipt = $("#jumlahreceipt").val();
        penerima = $("#penerima").val();
		base_url = $("#baseurl").val();
		$.post(base_url+"index.php/keuangan/receipt/save_new_item",{
			...
			savekdrekening:savekdrekening,kasbank:kasbank,costcenter:costcenter,personal:personal,jumlahreceipt:jumlahreceipt,
...
function deleteItem(kdrekening,id)
{
	if($("#transaksi").val()=="no"){
		...
		base_url = $("#baseurl").val();
		jumlahreceipt = $("#jumlahreceipt").val();
		$.post(base_url+"index.php/keuangan/receipt/delete_item",{
			no:no,kdrekening:kdrekening,urutan:id,jumlahreceipt:jumlahreceipt},
		function(data){
			$("#transaksi").val("no");
		});
	}
