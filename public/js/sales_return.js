...
function deleteTrans(nodok,url)
{
	...
	if (r==true)
	{
		window.location = url+"index.php/transaksi/sales_return/delete_trans/"+nodok;	
	}
	else
	{
  		return false;
...
function pickThis2(obj)
{
	base_url = $("#base_url").val();
	    ...
	    objek = obj.id;
		id = parseFloat(objek.substr(9,objek.length-9));
		url = base_url+"index.php/pop/pop_up_cari_delivery_order/index/0/1/";
		windowOpener(525, 1000, 'Cari Delivery No', url, 'Cari Delivery No')
