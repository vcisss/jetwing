...
if ( ! function_exists('form_dropdownDB_initDO')) //dengan Disabled options (option yg ga bisa di select
{
	function form_dropdownDB_initDO($name = '',$arrayData,$keydata,$value,$isactive, $selected='',$initval='0',$initoption='--Silakan Pilih--', $extra = '', $disable=''){
		if ($extra != '') $extra = ' '.$extra;
        ...
        $form .= '<option value="'.$initval.'"'.$disable.'>'.$initoption."</option>\n";
            for($i=0;$i<count($arrayData);$i++){
				...
				$disabled ='';
				if($arrayData [$i][$isactive] !='1') $disabled =' disabled';

                $sel = ($selected != $key) ? '' : ' selected="selected"';
                $form .= '<option value="'.$key.'"'.$sel.$disabled.'>'.$val."</option>\n";
...
if ( ! function_exists('form_dropdownDB_initJS'))
{
...
if ( ! function_exists('form_dropdownDB_initJSDO'))
{
    function form_dropdownDB_initJSDO($name = '',$arrayData,$keydata,$value,$isactive, $selected='',$initval='0',$initoption='--Silakan Pilih--', $extra = '', $disable=''){
        if ($extra != '') $extra = ' '.$extra;
        $form = '<select name="'.$name.'"'.$extra.">' \n";
        $form .= '+ \'<option value="'.$initval.'"'.$disable.'>'.$initoption."</option>'\n";
            for($i=0;$i<count($arrayData);$i++){
	...
	if ( ! function_exists('form_dropdownDB_options'))
{    
	function form_dropdownDB_options($arrayData,$keydata,$value) {
        $form	= '';
		for($i=0;$i<count($arrayData);$i++) {
			$key	= $arrayData[$i][$keydata];
			$val	= $arrayData[$i][$value];
...
if ( ! function_exists('form_dropdownDB_init'))
{   
