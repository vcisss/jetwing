...
<head>
    ...
    <script>
        ...
        function get_choose(stock,nama)
        {
            window.opener.document.forms["theform"]["v_groupid"].value = stock;
            window.opener.document.forms["theform"]["v_nama_stockid"].value = nama;
            self.close() ;
            return; 
        }
	</script>
		...
		<div id="table-2_wrapper" class="dataTables_wrapper form-inline" role="grid">
			...
			<table class="table table-bordered responsive">
				...
				foreach($stockdata as $val)
				{
					...
					?>
					<tr onclick="get_choose('<?php echo $val["GroupID"]; ?>','<?php echo $val["Nama"]; ?>')">
						<td><?php echo $val["StockID"] ?></td>
						<td><?php echo $val["VendorID"]; ?></td>
